...
import { getProductDetails, newReview, clearErrors, getRelatedProducts } from '../../Actions/productActions';
...
const ProductDetails = ({ match }) => {
    ...
    const { category,  _id: filteredProductId } = product;
    ...
    const relatedProducts = products && products.map(product => {
        if(product.category === category && product._id !== filteredProductId) return <Product key={product._id} product={product} col={4} />
    })
